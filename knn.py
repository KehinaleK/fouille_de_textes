...
def entrainement(regime, index):
    ...
    # Créer une pipeline
    model = make_pipeline(TfidfVectorizer(stop_words=french_stop_words), KNeighborsClassifier())
...
def obtention_phrases(test_evaluation, test_corpus, regime):
    ...
    with open(f"data/{regime}_knn.csv", "w") as fichier:
...
def matrice_de_confusion(test_evaluation, test_corpus, regime):
    ...
    sns.heatmap(matrice, square=True, annot=True, fmt='d',cmap='Blues', cbar=False, xticklabels=[f'non-{regime}', regime], yticklabels=[f'non-{regime}', regime])
